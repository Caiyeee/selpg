...
func main() {
	...
	if len(dest) > 0 {
		...
		Log.Stdout, err = subproc.StdinPipe()
		if err != nil {
			Log.ErrPrintf("Failed to send data to printer %s: %v\n", dest, err)
			os.Exit(1)
		}
	...
	if err == errStartOutOfRange {
		Log.ErrPrintf("start_page (%d) greater than total pages (%d), no output written\n", startPagenum, readPagenum)
	} else if err == errEndOutOfRange {
		Log.ErrPrintf("end_page (%d) greater than total pages (%d), less output than expected\n", endPagenum, readPagenum)
	} else if err != nil {
		Log.ErrPrintf("Unexpected error on page %d: %v\n", readPagenum, err)
		os.Exit(1)
	}

	if subproc != nil {
		Log.Stdout.(io.WriteCloser).Close()
		// await subprocess
		subproc.Wait()
	}
}
